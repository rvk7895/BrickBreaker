...
class Ball:
    def __init__(self):
        self.x = PADDLE_X + int(PADDLE_LENGTH/2) - 1
        self.y = PADDLE_Y - 1
        self.vel_x = 0
        self.vel_y = 0
        ...
        for brick in bricks:
            ...
            if self.vel_x > 0 and\
                (self.y <= brick.y + 1 and self.y >= brick.y - 1) and \
                    (self.x < brick.x_left and self.x + self.vel_x >= brick.x_left):
                self.x = brick.x_left - 1 - self.vel_x
            if self.vel_x < 0 and\
                (self.y <= brick.y + 1 and self.y >= brick.y - 1) and \
                    (self.x > brick.x_right and self.x + self.vel_x <= brick.x_right):
    ...
    def ball_paddle_collision(self, paddle):
        ...
        paddle_right_middle = paddle.x + int(paddle.length/2)
        if self.y == PADDLE_Y-1 and self.vel_y > 0 and self.x >= paddle.x \
            ...
            if self.x < paddle_left_middle:
                self.vel_x = int((self.x - paddle_left_middle + 1)/2)
            if self.x > paddle_right_middle:
                self.vel_x = int((self.x - paddle_right_middle + 1)/2)
    ...
    def ball_brick_collision(self, bricks=[], canvas=[]):
        the_brick = None
        collision = False
